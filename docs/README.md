...
- [x] DiscountAdapter : 할인을 적용하는 Adapter
    - [x] 만원 이하일때는 아무 할인 이벤트도 받지 못한다.
    - [x] Orders를 매개변수로 넣으면 할인의 이름과 총 할인 금액의 리스트를 반환한다.
        - [x] 할인 금액이 0원인 것은 리스트에서 제외한다.


- [ ] BadgeType : 뱃지의 타입과 넘어야하는 가격이 정의된 enumClass
    - 5천 원 이상: 별
    - 1만 원 이상: 트리
    - 2만 원 이상: 산타


- GoodsProvider : 상품들을 제공해주는 provider
    - [ ] 추가 증정 메뉴를 받기를 시도하는 함수를 가진다.
        - [ ] Product를 반환하도록 한다.
        - [ ] 120,000원이 넘으면 샴페인을 반환한다.
        - [ ] 총 금액에서 샴페인의 금액만큼 뺀다.
    - [ ] 총 구입 금액에 따라 다른 배지를 반환하는 함수를 가진다.
        - [ ] 가장 높은 것 하나만 반환해야 한다.
